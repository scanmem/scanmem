                              ...
                              char **fail_reason)
{
    ...
    if ( *arr_maxsz > maxsz / 2 ||
        (valarr_tmpptr = realloc(valarr, (*arr_maxsz *= 2) * sizeof(size_t)))
        == NULL)
        if ((valarr_tmpptr = realloc(valarr, (*arr_maxsz = maxsz) *
                                              sizeof(size_t))) == NULL) {
...
bool parse_uintset(const char *lptr, struct set *set, size_t maxsz)
{
    ...
    for (tok = lptr; *tok; tok++) {
        ...
        if (arr_szfilled == arr_maxsz && !inc_arr_sz(&valarr, &arr_maxsz,
                                                     maxsz, &fail_reason))
            goto error;
        ...
        if (isdigit(*tok)) {
            ...
            if (last_type == RANGE_TOK) {
                ...
                if ((arr_szfilled += (toknum - last_num)) > arr_maxsz)
                    while (arr_szfilled > arr_maxsz) {
                        if (!inc_arr_sz(&valarr, &arr_maxsz, maxsz, &fail_reason))
                            goto error;
                    }
